...
def calc_secondary(av,b):
    ...
    # Run calculations and populate dictionaries.
    b['vx'] = b['rovx'] / b['ro']
    b['vy'] = b['rovy'] / b['ro']
    b['v'] = np.sqrt(b['vx']**2 + b['vy']**2)
    b['t'] = (b['roe'] / b['ro'] - 0.5*b['v']**2) / av['cv']
    b['hstag'] = av['cp'] * b['t'] + 0.5 * b['v']**2
    b['tstag'] = b['t'] + 0.5 * b['v']**2 / av['cp']
    b['p'] = (b['roe'] - 0.5*b['ro'] * b['v']**2) * (av['gam'] - 1)
    b['pstag'] = b['p'] * np.power((b['tstag'] / b['t']),(av['gam'] / (av['gam'] - 1)))
    b['mach'] = b['v'] / np.sqrt(av['gam'] * av['rgas'] * b['t'])
    b['angle'] = np.arctan(b['vy'] / b['vx']) * 180/np.pi
    return b
