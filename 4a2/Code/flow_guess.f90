...
!     Variables required for the improved guess, you will need to add to these
      real :: l_i(g%ni)
!     INSERT
      real :: mflow, mach_lim, t_lim
      real :: v_guess(g%ni), ro_guess(g%ni), tstat(g%ni)
...
!     temperature and the exit velocity
      ...
      v_out = (2.0 * av%cp * (bcs%tstag - t_out))**0.5
...
!         INSERT
            ...
            v_guess(:) = mflow / (l_i(:) * ro_out)

            ! If static temperature below limit, select the limiting temperature
            tstat = max(t_lim, bcs%tstag - 0.5 * (v_guess(:)**2 / av%cp))

            ! If statement to identify locations where Mach exceeds limit
            if (minval(tstat) < t_lim) then
               write(6,*) 'Flow mach number is too high somewhere'
               write(6,*) 'Mach Number =', v_guess(minloc(tstat)) / &
                  sqrt(av%gam * av%rgas * tstat(minloc(tstat)))
               write(6,*) 'Location =', minloc(tstat)
            end if
            
            ro_guess(:) = ( bcs%pstag * (tstat(:) / bcs%tstag)**(1/av%fgam) ) &
                 / (av%rgas * tstat(:))
            v_guess = mflow / (l_i * ro_guess)
...
!         INSERT
            ...
            do i = 1, ni-1
               do j = 1, nj
                  ...
                  g%roe(i,j) = g%ro(i,j) * (av%cv * tstat(i) &
                       + 0.5 * v_guess(i)**2)
               end do
            end do
